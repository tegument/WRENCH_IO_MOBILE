...
def run_detached(args, wait=False):
	...
	if sys.platform.startswith('win'):
		...
		with open(os.devnull) as devnull:
			proc = Popen("bash -i -c '"+" ".join(args)+"'", stdout=devnull, stderr=STDOUT, shell=True)
			if wait:
				proc.wait()

def run_ctrlc_proof(args):
	# Run a process entirely detached from this one, impervious to Ctrl-C on the parent
	if sys.platform.startswith('win'):
		os.system("cmd /c start \"Detached Forge command\" \""+"\" \"".join(args)+"\"")
	else:
